...
clear all;
stim_amp=300; %minimum stim amplitude to take
stim_config='mono';
depth_thresh=8;
...
resp_dur=[-20,50];%[time_begin time_end] of the response vector, in ms
...
if ~unempty_exist('root_dir')
    ...
    %     root_dir='C:\rotem\hugo_data\HugoData-CtxThl on 132.64.163.150';
    root_dir='G:\users\ream\Prut\Yolanda\Data\YolandaData\';
end
...
for i_sess=1:length(sessions)
    ...
    for i_ss=1:length(SESSparam.SubSess)
        all_files=SESSparam.SubSess(i_ss).Files(1):SESSparam.SubSess(i_ss).Files(2);
            ...
            elseif monkey_let=='y'
                stim_files=[];
                for i_file=all_files %*** (see below)
                    if SESSparam.fileConfig(i_file).SCPStim && isequal(str2num(SESSparam.fileConfig(i_file).SCPStimAmp),stim_amp) &&...
                ...
                error('Wrong monkey letter, %s',monkey_let)
            end
        ...
        for i_file=stim_files
            ...
            if monkey_let=='h'
                do_site = isfield(SESSparam.SubSess(i_ss),'GlobalStim') && isfield(SESSparam.SubSess(i_ss).GlobalStim,'AmpMat') &&...
            ...
            elseif monkey_let=='y'
                if length(stim_times)>100
                    do_site = true; %The terms were checked in line ***
                else 
                    do_site=false;
                end
                ...
                for elec=elecs
                    ...
                    if four_elec
                        ...
                        [resp_vec,t] = get_resp_vec_fast(eval(DDFparam.Electrode(elec).Channel), ...
                            eval([DDFparam.Electrode(elec).Channel '_KHz']), stim_times,stim_times_Fs,resp_dur,highpass_f,signal_power,method,gauss_win_time);
                    else
                        ...
                        resp_vec = get_resp_vec_fast(eval(['Unit' num2str(elec)]), ...
                            eval(['Unit' num2str(elec) '_KHz']), stim_times,stim_times_Fs,resp_dur,highpass_f,signal_power,method,gauss_win_time);
                    end
...
save ('test_resp_med_mu', 'all_resp', 'all_resp_id' , 't')
