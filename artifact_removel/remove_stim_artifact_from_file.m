function [clean_signal,t,org_signal]=remove_stim_artifact_from_file(file_base_name,elect_num)
...
if any(strcmp(wvf_vars,elect_var_name))
	raw_signal = load(wvf_file,elect_var_name,[elect_var_name '_KHz']);
elseif any(strcmp(bhv_vars,elect_var_name))
...
if any(strcmp(bhv_vars,'AMstim_on'))
	stim_times=load(bhv_file,'AMstim_on','AMstim_on_KHz');
elseif any(strcmp(wvf_vars,'AMstim_on'))
...
clean_signal=remove_stim_artifact(...
	raw_signal.(elect_var_name),raw_signal.([elect_var_name '_KHz']),stim_times.AMstim_on,stim_times.AMstim_on_KHz); 

if nargout>=2
	t=[1:length(raw_signal.(elect_var_name))]/raw_signal.([elect_var_name '_KHz']);
	org_signal = raw_signal.(elect_var_name);
end